...
ALTER PROCEDURE [dbo].[VerConceptoCobroDePropiedad]
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Si llega acá, se buscan los conceptos de cobro
        SELECT CC.[nombre] AS 'Nombre', CCdP.[fechaInicio] AS 'Inicio'
        FROM [dbo].[ConceptoCobroDePropiedad] CCdP
        INNER JOIN [dbo].[Propiedad] P
            ON CCdP.idPropiedad = P.id
        INNER JOIN [dbo].[ConceptoCobro] CC
            ON CCdP.[idConceptoCobro] = CC.[id]
        WHERE CCdP.idPropiedad = @idPropiedad
            AND CCdP.fechaFin IS NULL; -- NULL = sigue activa la relación
