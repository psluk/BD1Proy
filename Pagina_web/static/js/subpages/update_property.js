...
const actualizarApi = './post/property_update'
const propiedadApi = './get/property/';
...
function datosDePropiedad(solicitudPropiedad) {
    // Función que espera hasta que el servidor retorna la información
    if (solicitudPropiedad.readyState < 4 || document.getElementById(listaUsos).disabled) {
        // 4 = listo
        setTimeout(function () { datosDePropiedad(solicitudPropiedad); }, 100)
        // 100 = revisa cada 100 milisegundos
    } else {
        let datos = JSON.parse(solicitudPropiedad.response);
        if (datos.status != 0) {
            alert("Ocurrió un error.\n\nRevise que la propiedad exista e inténtelo nuevamente.");
        } else {
            document.getElementById(finca).value = datos.results[0].numeroFinca;
            document.getElementById(area).value = datos.results[0].area;
            document.getElementById(medidor).value = datos.results[0].numeroMedidor;
            document.getElementById(listaUsos).value = datos.results[0].uso;
            document.getElementById(listaZonas).value = datos.results[0].zona;
            document.getElementById(valor).value = datos.results[0].valorFiscal;
        }
...
window.addEventListener('load', function () {
    ...
    if (!propiedadBuscada) {
        alert("Número de propiedad no ingresado");
    } else {
        obtenerZonas();
        obtenerUsos();
        let solicitudPropiedad = new XMLHttpRequest();
        solicitudPropiedad.open('GET', propiedadApi + propiedadBuscada, false);   // asincrónico
        solicitudPropiedad.send();
    ...
    if (document.getElementById(formId)) {
        document.getElementById(formId).onsubmit = function () {
            ...
            solicitudCrear.open("POST", actualizarApi, true);
